...
DESTINATION = DestinationConfig(
    ...
    name_variants=[
        "hòa bình", "hoà bình", "hoa binh"
    ],

    entities=[
        "thung nai", "mai châu", "mai chau", "đèo đá trắng", "thuỷ điện hoà bình", "thủy điện hòa bình",
        "suối khoáng kim bôi", "kim bôi", "kim boi", "bản lác", "ban lac", 
        "thác bờ", "thac bo", "động đá bạc", "lũng vân", "lung van", "cửu thác tú sơn",
        "động tiên phi", "bảo tàng không gian văn hóa mường", "đèo thung khe"
    ],

    districts=[
        "thành phố hòa bình", "thành phố hoà bình", "lương sơn", "luong son", "cao phong", "đà bắc", "da bac",
        "kim bôi", "kim boi", "kỳ sơn", "ky son", "lạc sơn", "lac son", "lạc thủy", "lac thuy", "lạc thuỷ",
        "mai châu", "mai chau", "tân lạc", "tan lac", "yên thủy", "yen thuy", "yên thuỷ"
    ],

    landmark_keywords=[
        "thung nai", "mai chau", "deo da trang", "thuy dien hoa binh",
        "kim boi", "ban lac", "thac bo", "dong da bac", "lung van",
        "cuu thac tu son", "dong tien phi", "bao tang muong", "deo thung khe",
        "thac mu", "hang kia", "pa co"
    ],

    cluster_rules={
        "Địa danh tham quan / Văn hóa - Lịch sử": [
            {"pattern": r"(thung nai|den ba chua thac bo|thac bo)", "name": "Thung Nai & Thác Bờ"},
            {"pattern": r"(mai chau|ban lac|hang kia|pa co)", "name": "Mai Châu"},
            {"pattern": r"(thuong tien|kim boi|suoi khoang)", "name": "Suối khoáng Kim Bôi"},
            {"pattern": r"(deo da trang|thung khe)", "name": "Đèo Đá Trắng (Thung Khe)"},
            {"pattern": r"(thuy dien)", "name": "Thủy điện Hòa Bình"},
            {"pattern": r"(lung van|cao phong|thac mu)", "name": "Các điểm tham quan khác Hòa Bình"},
        ],
        "Lưu trú": [
            {"pattern": r"(resort|serena|bakhan|mai chau hideaway|ivory|avana)", "name": "Resort Hòa Bình"},
            {"pattern": r"(homestay)", "name": "Homestay Hòa Bình"},
            {"pattern": r"(\\b4 sao\\b|4 sao|\\b5 sao\\b|5 sao)", "name": "Khách sạn cao cấp Hòa Bình"},
        ],
        "Di chuyển": [
            {"pattern": r"(xe khách|xe khach|limousine|xe tuyen|ben xe)", "name": "Xe khách/Limousine đi Hòa Bình"},
            {"pattern": r"\\btour\\b", "name": "Tour Hòa Bình"},
        ],
        "Ẩm thực (F&B)": [
            {"pattern": r"(ca song da|cá sông đà|lon ban|lợn bản|cam cao phong|com lam|cơm lam)", "name": "Đặc sản Hòa Bình"},
        ],
    },
...
def cluster_type(keyword: str) -> str:
    ...
    if re.search(r"(khach san|hotel|resort|homestay|hostel|nha nghi|villa|apartment|can ho|ecolodge|hideaway|retreat|bungalow|lodge)", kw_n):
        return "Lưu trú"
...
def infer_intent(keyword: str) -> str:
    ...
    comm_sig = re.search(
        r"(review|top|tot nhat|tốt nhất|so sanh|so sánh|kinh nghiem|kinh nghiệm|nen di|nên đi|list|nen o|nên ở|co gi choi|có gì chơi)",
        kw_n,
    )

    is_stay = re.search(r"(khach san|hotel|resort|homestay|hostel|nha nghi|villa|apartment|can ho|ecolodge|hideaway|retreat|bungalow|lodge)", kw_n)
    is_move = re.search(
...
def build_candidates():
    ...
    for r in gsc_rows:
        ...
        # Lọc bỏ từ khoá nhạy cảm/không phù hợp
