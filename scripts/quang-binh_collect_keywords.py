...
DESTINATION = DestinationConfig(
    ...
    name_variants=[
        ...
        "đồng hới", "dong hoi", "donghoi"
    ],

    entities=[
        "phong nha", "kẻ bàng", "ke bang", "sơn đoòng", "son doong", 
        "động thiên đường", "thiên đường", "biển nhật lệ", "nhat le", 
        "vũng chùa", "đảo yến", "bãi đá nhảy", "suối moọc", "sông chày", 
        "hang én", "tú làn", "vực thếch", "bảo tàng", "hải đăng"
    ],
    ...
    landmark_keywords=[
        "phong nha", "ke bang", "son doong", "thien duong", "nhat le", 
        "vung chua", "dao yen", "da nhay", "suoi mooc", "song chay", 
        "hang en", "tu lan"
    ],

    cluster_rules={
        "Địa danh tham quan / Văn hóa - Lịch sử": [
            {"pattern": r"(phong nha)", "name": "Động Phong Nha"},
            {"pattern": r"(son doong)", "name": "Hang Sơn Đoòng"},
            {"pattern": r"(thien duong)", "name": "Động Thiên Đường"},
            {"pattern": r"(suoi mooc)", "name": "Suối nước Moọc"},
            {"pattern": r"(song chay|hang toi)", "name": "Sông Chày - Hang Tối"},
            {"pattern": r"(da nhay)", "name": "Bãi Đá Nhảy"},
            {"pattern": r"(vung chua|dao yen)", "name": "Vũng Chùa - Đảo Yến"},
            {"pattern": r"(nhat le)", "name": "Biển Nhật Lệ"},
        ],
        "Lưu trú": [
            {"pattern": r"(\\b5 sao\\b)", "name": "Khách sạn 5 sao Quảng Bình"},
            {"pattern": r"(\\b4 sao\\b)", "name": "Khách sạn 4 sao Quảng Bình"},
            {"pattern": r"(phong nha)", "name": "Khách sạn/Homestay Phong Nha"},
            {"pattern": r"(dong hoi)", "name": "Khách sạn Đồng Hới"},
        ],
        "Di chuyển": [
            {"pattern": r"(san bay|airport)", "name": "Sân bay Đồng Hới"},
            {"pattern": r"(ga dong hoi)", "name": "Ga Đồng Hới"},
        ],
        "Ẩm thực (F&B)": [
            {"pattern": r"(hai san)", "name": "Hải sản Quảng Bình"},
            {"pattern": r"(chao canh)", "name": "Cháo canh Quảng Bình"},
        ],
    },
    ...
    extra_travel_patterns="|hang|dong|kham pha|trekking",
)
...
# Danh sách từ khoá cần loại bỏ (không phù hợp với chiến lược SEO du lịch)
EXCLUDED_TERMS = [
    "nha nghi",  # Nhà nghỉ — không phù hợp brand du lịch cao cấp
]

def is_excluded(kw_n: str) -> bool:
    """Kiểm tra keyword có chứa term bị loại trừ hay không."""
...
def build_candidates():
    ...
    for r in gsc_rows:
        ...
        # Lọc bỏ từ khoá trong danh sách excluded (nhà nghỉ, ...)
        if is_excluded(kw_n):
            continue
