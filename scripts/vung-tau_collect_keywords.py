...
DESTINATION = DestinationConfig(
    ...
    entities=[
        "hồ tràm", "ho tram", "bãi trước", "bai truoc", "bãi sau", "bai sau",
        "tượng chúa giang tay", "tuong chua giang tay", "hòn bà", "hon ba",
        "mũi nghinh phong", "mui nghinh phong", "côn đảo", "con dao",
        "long hải", "long hai", "bạch dinh", "bach dinh",
        "hồ cốc", "ho coc", "núi lớn", "nui lon", "núi nhỏ", "nui nho", "phước hải", "phuoc hai", "suối ồ", "suoi o"
    ],
    districts=[
        "xuyên mộc", "xuyen moc", "đất đỏ", "dat do", "châu đức", "chau duc",
        "long điền", "long dien"
    ],
    landmark_keywords=[
        "ho tram", "bai truoc", "bai sau", "chua", "hon ba", "mui nghinh phong",
        "con dao", "long hai", "bach dinh", "ho coc", "nui lon", "nui nho", "phuoc hai", "suoi o"
    ],
    cluster_rules={
        "Địa danh tham quan / Văn hóa - Lịch sử": [
            {"pattern": r"(ho tram|hồ tràm)", "name": "Hồ Tràm"},
            {"pattern": r"(con dao|côn đảo)", "name": "Côn Đảo"},
            {"pattern": r"(long hai|long hải)", "name": "Long Hải"},
            {"pattern": r"(bai sau|bãi sau)", "name": "Bãi Sau"},
            {"pattern": r"(bai truoc|bãi trước)", "name": "Bãi Trước"},
            {"pattern": r"(mui nghinh phong|mũi nghinh phong)", "name": "Mũi Nghinh Phong"},
        ],
        "Lưu trú": [
            {"pattern": r"(\\b4 sao\\b|4 sao)", "name": "Khách sạn 4 sao Vũng Tàu"},
            {"pattern": r"(\\b5 sao\\b|5 sao)", "name": "Khách sạn 5 sao Vũng Tàu"},
            {"pattern": r"(resort)", "name": "Resort Vũng Tàu"},
            {"pattern": r"(homestay)", "name": "Homestay Vũng Tàu"},
            {"pattern": r"(villa)", "name": "Villa Vũng Tàu"},
        ],
        "Di chuyển": [
            {"pattern": r"(san bay|airport)", "name": "Sân bay & di chuyển Vũng Tàu"},
            {"pattern": r"\\btour\\b", "name": "Tour Vũng Tàu"},
            {"pattern": r"(limousine|xe khach|thue xe)", "name": "Thuê xe & di chuyển Vũng Tàu"},
        ],
        "Ẩm thực (F&B)": [
            {"pattern": r"(hai san|hải sản)", "name": "Hải sản Vũng Tàu"},
            {"pattern": r"(lanh|bánh khọt)", "name": "Bánh khọt Vũng Tàu"},
        ],
    },
    province_display_overrides={
        "vung tau": "Vũng Tàu",
    },
    extra_travel_patterns="|bien|bai bien|tam bien|ngam san ho|ca no",
)
...
def is_destination_related(keyword: str, geo_label: str) -> bool:
    ...
    """
    kw_n = norm(keyword)
    # Bỏ qua các địa điểm out-of-scope theo yêu cầu (Hà Nam, Huế, Bắc Ninh, Lăng Cô, Thái Bình)
    if any(p in kw_n for p in ["ha nam", "hue", "bac ninh", "lang co", "thai binh"]):
        return False
...
def build_candidates():
    ...
    for r in gsc_rows:
        ...
        # Lọc bỏ từ khoá nhạy cảm/không phù hợp (tình yêu, tình nhân, nhà nghỉ, phòng trọ), năm, và đối thủ cạnh tranh
        if re.search(r"(tinh yeu|tinh nhan|nha nghi|phong tro|chudu|trivago|saigontourist|\b20[1-3][0-9]\b)", kw_n):
            continue
    ...
    for kw_n, t in trends.items():
        ...
        # Lọc bỏ từ khoá nhạy cảm/không phù hợp và năm
        if re.search(r"(tinh yeu|tinh nhan|nha nghi|phong tro|\b20[1-3][0-9]\b)", kw_n):
            continue
