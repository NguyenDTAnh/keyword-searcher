...
DESTINATION = DestinationConfig(
    ...
    name_variants=[
        # Có dấu + không dấu + viết liền
        "cửa lò", "cua lo", "cualo",
        "cửa lò nghệ an", "cua lo nghe an",
    ],

    entities=[
        # Entity đặc hữu — địa danh/khách sạn nổi bật chỉ thuộc Cửa Lò
        "bãi lữ", "bai lu",
        "hòn ngư", "hon ngu",
        "đảo lan châu", "dao lan chau", "lan chau",
        "đảo ngư", "dao ngu",
        "mường thanh cửa lò", "muong thanh cua lo",
        "summer hotel cửa lò", "summer cua lo",
        "vinpearl cửa lò", "vinpearl cua lo",
        "núi quyết", "nui quyet",
        "đền cuông", "den cuong",
    ],

    districts=[
        # Các phường thuộc Cửa Lò
        "nghi thu", "nghi thủy",
        "nghi tân", "nghi hòa", "nghi hải",
        "thu thủy",
    ],

    landmark_keywords=[
        # Dạng KHÔNG DẤU để match normalized
        "bai lu", "hon ngu", "dao lan chau", "lan chau",
        "dao ngu", "nui quyet", "den cuong",
        "cap treo", "cau cua lo",
        "cong vien nuoc", "quang truong",
    ],

    cluster_rules={
        "Địa danh tham quan / Văn hóa - Lịch sử": [
            {"pattern": r"(bai lu|bãi lữ)", "name": "Bãi Lữ Resort"},
            {"pattern": r"(hon ngu|hòn ngư)", "name": "Đảo Hòn Ngư"},
            {"pattern": r"(lan chau|lan châu)", "name": "Đảo Lan Châu"},
            {"pattern": r"(dao ngu|đảo ngư)", "name": "Đảo Ngư"},
            {"pattern": r"(den cuong|đền cuông)", "name": "Đền Cuông"},
            {"pattern": r"(nui quyet|núi quyết)", "name": "Núi Quyết"},
            {"pattern": r"(cap treo)", "name": "Cáp treo Cửa Lò"},
            {"pattern": r"(cong vien nuoc|công viên nước)", "name": "Công viên nước Cửa Lò"},
        ],
        "Lưu trú": [
            {"pattern": r"(muong thanh|mường thanh)", "name": "Khách sạn Mường Thanh Cửa Lò"},
            {"pattern": r"(summer)", "name": "Khách sạn Summer Cửa Lò"},
            {"pattern": r"(vinpearl)", "name": "Vinpearl Cửa Lò"},
            {"pattern": r"(blue wave)", "name": "Blue Wave Hotel Cửa Lò"},
            {"pattern": r"(sai gon kim lien|sài gòn kim liên)", "name": "KS Sài Gòn Kim Liên Cửa Lò"},
            {"pattern": r"(nui hong|núi hồng)", "name": "Khách sạn Núi Hồng Cửa Lò"},
            {"pattern": r"(bank star)", "name": "Bank Star Hotel Cửa Lò"},
            {"pattern": r"(paris hotel|b&h paris)", "name": "B&H Paris Hotel Cửa Lò"},
            {"pattern": r"(\b3 sao\b|3 sao)", "name": "Khách sạn 3 sao Cửa Lò"},
            {"pattern": r"(\b4 sao\b|4 sao)", "name": "Khách sạn 4 sao Cửa Lò"},
            {"pattern": r"(\b5 sao\b|5 sao)", "name": "Khách sạn 5 sao Cửa Lò"},
        ],
        "Di chuyển": [
            {"pattern": r"(taxi)", "name": "Taxi Cửa Lò"},
            {"pattern": r"\\btour\\b", "name": "Tour Cửa Lò"},
        ],
        "Ẩm thực (F&B)": [
            {"pattern": r"(hai san|hải sản)", "name": "Hải sản Cửa Lò"},
            {"pattern": r"(dac san|đặc sản)", "name": "Đặc sản Cửa Lò"},
        ],
    },
    ...
    extra_travel_patterns=r"|bien|bai bien|bai tam|tam bien|san golf|golf|be boi|cong vien nuoc",
)
...
def build_candidates():
    ...
    for kw_n, p in planner_data.items():
        ...
        # Hỗ trợ cả tiếng Việt (Thấp/Trung bình/Cao) từ Planner locale VN
        comp_val = norm(p["competition"] or "")
        if comp_val in ("low", "thap"):
            comp_factor = 1.5
        elif comp_val in ("medium", "trung binh"):
            comp_factor = 1.2
        elif comp_val in ("high", "cao"):
            comp_factor = 0.8
        else:
            comp_factor = 1.0
