...
DESTINATION = DestinationConfig(
    ...
    entities=[
        # Phố cổ & di sản (KHÔNG dùng "phố cổ" standalone vì match nhầm "phố cổ hà nội")
        "phố cổ hội an", "pho co hoi an",
        "chùa cầu", "chua cau",
        "hội quán phúc kiến", "hoi quan phuc kien",
        "hội quán quảng đông", "hoi quan quang dong",
        # Biển & đảo
        "cù lao chàm", "cu lao cham",
        "bãi biển an bàng", "bai bien an bang", "an bàng", "an bang",
        "cửa đại", "cua dai",
        # Làng nghề
        "làng rau trà quế", "lang rau tra que", "trà quế", "tra que",
        "làng gốm thanh hà", "lang gom thanh ha", "thanh hà", "thanh ha",
        "làng mộc kim bồng", "lang moc kim bong", "kim bồng", "kim bong",
        # Khác
        "chợ hội an", "cho hoi an",
        "đêm hội an", "dem hoi an",
        "sông hoài", "song hoai",
        "rừng dừa bảy mẫu", "rung dua bay mau",
    ],

    districts=[
        # Bỏ "cẩm an", "minh an", "tân an" vì quá ngắn, dễ match nhầm
        # Bỏ "thanh hà" vì trùng entity (đã có trong entities dạng "làng gốm thanh hà")
        "cẩm châu", "cẩm hà", "cẩm kim", "cẩm nam", "cẩm phô",
        "sơn phong",
        "cẩm thanh", "điện bàn", "điện dương",
    ],

    landmark_keywords=[
        "pho co", "chua cau", "cu lao cham", "an bang", "cua dai",
        "tra que", "thanh ha", "kim bong", "song hoai",
        "rung dua bay mau", "hoi quan phuc kien", "hoi quan quang dong",
        "cho hoi an", "dem hoi an",
    ],

    cluster_rules={
        "Địa danh tham quan / Văn hóa - Lịch sử": [
            {"pattern": r"(pho co)", "name": "Phố cổ Hội An"},
            {"pattern": r"(chua cau)", "name": "Chùa Cầu"},
            {"pattern": r"(cu lao cham)", "name": "Cù Lao Chàm"},
            {"pattern": r"(rung dua|bay mau)", "name": "Rừng dừa Bảy Mẫu"},
            {"pattern": r"(hoi quan)", "name": "Hội quán Hội An"},
            {"pattern": r"(tra que)", "name": "Làng rau Trà Quế"},
            {"pattern": r"(thanh ha|gom)", "name": "Làng gốm Thanh Hà"},
            {"pattern": r"(kim bong)", "name": "Làng mộc Kim Bồng"},
            {"pattern": r"(song hoai)", "name": "Sông Hoài"},
        ],
        "Lưu trú": [
            {"pattern": r"(an bang)", "name": "Khách sạn gần An Bàng"},
            {"pattern": r"(cua dai)", "name": "Khách sạn gần Cửa Đại"},
            {"pattern": r"(resort)", "name": "Resort Hội An"},
            {"pattern": r"(\b4 sao\b|4 sao)", "name": "Khách sạn 4 sao Hội An"},
            {"pattern": r"(\b5 sao\b|5 sao)", "name": "Khách sạn 5 sao Hội An"},
            {"pattern": r"(homestay)", "name": "Homestay Hội An"},
            {"pattern": r"(villa)", "name": "Villa Hội An"},
            {"pattern": r"(khach san|hotel)", "name": "Khách sạn Hội An"},
        ],
        "Di chuyển": [
            {"pattern": r"(san bay|airport)", "name": "Sân bay & di chuyển Hội An"},
            {"pattern": r"(ve may bay|may bay)", "name": "Vé máy bay đi Hội An"},
            {"pattern": r"(thue xe may|xe may)", "name": "Thuê xe máy Hội An"},
            {"pattern": r"(xe khach|limousine|giuong nam)", "name": "Xe khách đi Hội An"},
            {"pattern": r"\btour\b", "name": "Tour Hội An"},
            {"pattern": r"(da nang.*hoi an|hoi an.*da nang)", "name": "Di chuyển Đà Nẵng - Hội An"},
        ],
        "Ẩm thực (F&B)": [
            {"pattern": r"(cao lau)", "name": "Cao lầu Hội An"},
            {"pattern": r"(mi quang|mi quảng)", "name": "Mì Quảng Hội An"},
            {"pattern": r"(com ga|cơm gà)", "name": "Cơm gà Hội An"},
            {"pattern": r"(banh mi|bánh mì)", "name": "Bánh mì Hội An"},
            {"pattern": r"(banh bao|banh vac|white rose)", "name": "Bánh bao/Bánh vạc Hội An"},
            {"pattern": r"(hai san|hải sản)", "name": "Hải sản Hội An"},
            {"pattern": r"(quan an|nha hang)", "name": "Quán ăn ngon Hội An"},
        ],
    },
    ...
    extra_travel_patterns="|bien|bai bien|lan|diving|snorkeling|pho co|den long|lantern",
)
...
def build_candidates():
    ...
    for r in gsc_rows:
        ...
        if re.search(r"(tinh yeu|tinh nhan|nha nghi|vietravel|viettravel|agoda)", kw_n):
            continue
